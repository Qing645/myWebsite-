...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
    // const query = querystring.parse(options.url);以弃用
    const urlSp = new URLSearchParams(options.url).get("limit");
    return Mock.mock({
        code: 0,
        msg: "",
        data: {
...
Mock.mock("/api/comment", "post",function (arg) {
...
Mock.mock(/^\/api\/comment\/?(\?.+)?$/, "get", function (options) {
    const url = new URLSearchParams(options.url).get("limit");
    return Mock.mock({
        code: 0,
        msg: "",
        data: {
